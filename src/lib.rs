...
pub fn logefix(x: u32, precision: u8) -> i32 {
    let t = (log2fix(x, precision) as u64).overflowing_mul(INV_LOG2_E_Q1DOT31).0;
    (t >> 31) as i32
}

pub fn log10fix(x: u32, precision: u8) -> i32 {
    let t = (log2fix(x, precision) as u64).overflowing_mul(INV_LOG2_10_Q1DOT31).0;
    (t >> 31) as i32
}
